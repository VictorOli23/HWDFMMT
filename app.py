...
# ==========================================
# 1. BANCO DE DADOS LOCAL (CRC PERSISTENTE)
# ==========================================
def init_db():
...
# Inicialização de Variáveis na Sessão (Memória do Streamlit)
...
if "df_fmt_previous" not in st.session_state:
    st.session_state.df_fmt_previous = pd.DataFrame() # Memória para o Handover
...
if menu == "📥 Upload & Processamento":
    ...
    with col1:
        ...
        f_fmmt = st.file_uploader("2. Base Móvel FMMT", type=["xlsx", "csv"])
        f_b2b = st.file_uploader("3. Base B2B (Corporativo)", type=["xlsx", "csv"])

    with col2:
        st.subheader("Bases de Apoio & Correlação")
        f_grafana = st.file_uploader("4. Base Grafana (Alarmes CSV)", type=["xlsx", "csv"])
        f_quadrantes = st.file_uploader("5. Base de Quadrantes (QD / ANF)", type=["xlsx", "csv"])
        f_crc = st.file_uploader("6. Base CRC (Histórico)", type=["xlsx", "csv"])

    if st.button("🚀 Processar e Integrar Todas as Bases", type="primary", use_container_width=True):
        if not f_fmt:
            st.error("A Base Total FMT é obrigatória.")
        else:
            with st.spinner("Lendo planilhas, mapeando colunas e cruzando alarmes de Anéis..."):
                df_fmt_raw = load_file(f_fmt, ["BACKLOG", "FMT", "TASK", "EVENTO"])
                df_fmmt = load_file(f_fmmt, ["FMMT", "MOVEL"])
                df_b2b_raw = load_file(f_b2b, ["B2B", "CORPORATIVO"])
                df_graf = load_file(f_grafana, ["GRAFANA", "ANEIS", "ALARMES"])
                df_quad = load_file(f_quadrantes, ["QUAD", "QD", "ANF"])
                ...
                if not df_quad.empty:
                    col_q_end = next((c for c in df_quad.columns if "END" in str(c).upper()), df_quad.columns[0])
                    col_q_val = next((c for c in df_quad.columns if any(k in str(c).upper() for k in ["QD", "QUADRANTE", "ANF"])), df_quad.columns[-1])
                    quad_map = dict(zip(df_quad[col_q_end].astype(str).str.strip().str.upper(), df_quad[col_q_val].astype(str).str.strip()))
                    ...
                    df_fmt["QUADRANTE"] = df_fmt["END_ID"].astype(str).apply(lambda x: re.search(r'(QD\s*\d+|ANF\s*\d+)', str(x), re.IGNORECASE).group(0).upper() if re.search(r'(QD\s*\d+|ANF\s*\d+)', str(x), re.IGNORECASE) else "N/A")
                ...
                df_fmt["IS_CRC"] = df_fmt["TSK"].astype(str).str.strip().apply(lambda x: "SIM" if x in crc_tsks else "NÃO")

                if not df_b2b_raw.empty:
                    ...
                    # CORREÇÃO APLICADA AQUI: .str.upper() em vez de .upper()
                ...
                # LÓGICA DE MEMÓRIA PARA ENTRANTES/SAINTES:
                ...
                st.success(f"✅ Processamento concluído! {len(df_fmt)} registros lidos. Encontrados {aneis_count} Anéis Abertos cruzando com Grafana.")

# ==========================================
# ABA 2: BACKLOG OPERACIONAL
# ==========================================
elif menu == "📂 Backlog Operacional":
    st.title("📂 Backlog Operacional")
    st.caption("Visão operacional espelhada na planilha padrão da equipe.")
    ...
    if df.empty:
        ...
        with col_b1:
            if st.button("💾 Salvar Alterações", type="primary", use_container_width=True):
                ...
                st.success("✅ Backlog atualizado com sucesso!")
                st.rerun()

        with col_b2:
            output = io.BytesIO()
            with pd.ExcelWriter(output, engine="openpyxl") as writer:
                df_bk_view[cols_backlog].to_excel(writer, index=False, sheet_name="Backlog")
            excel_data = output.getvalue()
            st.download_button(
                label="📥 Baixar Backlog em Excel (.xlsx)",
                data=excel_data,
                file_name=f"Backlog_FMT_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
            )

# ==========================================
# ABA 3: HANDOVER (SAINTES E ENTRANTES) - NOVA
# ==========================================
